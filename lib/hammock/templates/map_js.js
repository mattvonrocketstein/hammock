if (GBrowserIsCompatible()) {
    ...
    function do_label(_id, lat, lon){
            ...
            if(e.which == 13){
                $.ajax({
                        type: "get",
                            ...
                            url: "/set_label",
                            success: sfunc_factory('#label_dialog'),
                            error: efunc
                            });
            }
        }
        ...
        $('#label_input ').bind('keypress',keypress_event);
        $('#label_input ').focus();
    }

    function do_tag(_id, tag){
        function tag_keypress_event(e){
            if(e.which == 13){
                $.ajax({
                        type: "get",
                            data: {id:_id,
                                tag:$('#tag_input#').val()},
                            url: '/set_tag',
                            success: sfunc_factory('#tag_dialog'),
                            error: efunc
                            });
            }
        }
        ...
        $('#tag_input ').bind('keypress',tag_keypress_event);
        $('#tag_input ').focus();
    }
