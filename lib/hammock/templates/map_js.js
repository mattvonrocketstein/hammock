if (GBrowserIsCompatible()) {
    ...
    function createMarker(point,html,main_tag) {
        //alert(main_tag);
        ...
        var iconf = "/static/markers/yellow_MarkerC.png";
    ...
    map.setCenter(new GLatLng({{center_lat}}, {{center_lon}}), {{center_zoom}});

    {% for lat,lon,label,tags in points %}
    var marker = createMarker(new GLatLng({{lat}},{{lon}}),'{{label}}','{{tags.0}}');
    map.addOverlay(marker);
    {% endfor %}
    ...
    {% if minLat %}
    // set page does not have minLat yet
    var minLat={{minLat}};
    var minLng={{minLng}};
    var maxLat={{maxLat}};
    var maxLng={{maxLng}};
